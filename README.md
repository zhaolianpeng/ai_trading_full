...
- 🛡️ **强制过滤器**（提升胜率）：
  - ATR过滤（避免低波动）
  - **EMA排列**（强制要求，双向交易）：
    - **做多信号**：EMA多头排列（ema21 > ema55 > ema100）
    - **做空信号**：EMA空头排列（ema21 < ema55 < ema100）
...
- **EMA 排列**（强制要求，双向交易）：
  - **做多信号**：要求 EMA 多头排列 `ema21 > ema55 > ema100`，确保只在明确的上升趋势中交易
  - **做空信号**：要求 EMA 空头排列 `ema21 < ema55 < ema100`，确保只在明确的下降趋势中交易
...
#### 基础指标评分
- **趋势确认** (+20分)：
  - **做多信号**：EMA 多头排列（ema21 > ema55 > ema100）
  - **做空信号**：EMA 空头排列（ema21 < ema55 < ema100）
- **价格位置确认** (+15分)：
  - **做多信号**：价格在 EMA 上方
  - **做空信号**：价格在 EMA 下方
- **成交量确认** (+15-20分)：成交量放大
- **RSI 确认** (+5-10分)：
  - **做多信号**：RSI 健康区间（40-70）或超卖（<30）
  - **做空信号**：RSI 健康区间（30-60）或超买（>70）
- **价格动量确认** (+10分)：
  - **做多信号**：5周期正动量
  - **做空信号**：5周期负动量
- **支撑位确认** (+15分)：接近支撑位但未跌破
...
#### 高级指标评分（如果可用）
- **MACD 确认** (+15分)：
  - **做多信号**：MACD 多头（macd > macd_signal）
  - **做空信号**：MACD 空头（macd < macd_signal）
- **布林带确认** (+10分)：
  - **做多信号**：价格在布林带中轨上方
  - **做空信号**：价格在布林带中轨下方
- **ATR 波动率确认** (+10分 或 -10分)：波动率正常
- **随机指标确认** (+10分)：
  - **做多信号**：随机指标金叉（K > D）且未超买（K < 80）
  - **做空信号**：随机指标死叉（K < D）且未超卖（K > 20）
- **CCI 确认** (+10分)：
  - **做多信号**：CCI 健康区间（0-100）或超卖（< -100）
  - **做空信号**：CCI 健康区间（-100-0）或超买（> 100）
...
- **Donchian通道确认** (+10分)：
  - **做多信号**：Donchian通道上升趋势（UP）
  - **做空信号**：Donchian通道下降趋势（DOWN）
...
- 强制要求：
  - **做多信号**：EMA多头排列、成交量放大≥1.2倍、趋势强度≥60
  - **做空信号**：EMA空头排列、成交量放大≥1.2倍、趋势强度≥60
